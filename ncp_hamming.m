...
% 
% (slightly modified by Katharina Glomb to compute Hamming distances, to be
% used with binary tensors)
%
...
function [F,iter,relerr,sigerr,REC]=ncp_hamming(X,r,varargin)
	% set parameters
	params = inputParser;
        ...
        if (iter >= par.min_iter)
            if (par.verbose && (tTotal > par.max_time)) || (~par.verbose && ((cputime-tStart)>par.max_time))
				...
				switch par.stop_criterion
					...
					case 2
                        [ver(1).rel_Error,ver(1).sig_Error] = getRelError(X,F_kten);
    ...
    [final.rel_Error,final.sig_Error] = getRelError(X,F_kten);
    relerr = final.rel_Error;
    sigerr = final.sig_Error;
	REC.final = final;
...
function ver = prepareHIS(ver,X,F,F_kten,prev_F,pGrad,init,par,iter,elapsed)
	...
	if ~isfield(ver,'rel_Error')
		[ver.rel_Error,ver.sig_Error]= getRelError(X,F_kten);
	end
	for i=1:par.nWay
...
function [rel_Error,sig_Error] = getRelError(X,F_kten)
